...
XColor GraphicWindow::parse_color(string color) const {
   // parse color
   Colormap colormap;
   colormap = DefaultColormap(display, 0);
   XColor x_color;
   char *char_color = new char[color.size() + 1];
   char_color[color.size()] = 0;
   memcpy(char_color, color.c_str(), color.size());

   XParseColor(display, colormap, char_color, &x_color);
   XAllocColor(display, colormap, &x_color);

   return x_color;
}

void GraphicWindow::point(double x, double y, string color)
{  
   const int POINT_RADIUS = 3;
   int disp_x = user_to_disp_x(x);
   int disp_y = user_to_disp_y(y);

   XSetForeground(display, xgc, parse_color(color).pixel);
...
{  
	GraphicWindow::point(x, y, "#000000");
}

void GraphicWindow::ellipse(double x, double y, double ra, double rb, string color)
{  
   int disp_x = user_to_disp_x(x);
   int disp_y = user_to_disp_y(y);
...
{  
   GraphicWindow::ellipse(x, y, ra, rb, "#000000");
}

void GraphicWindow::line(double xfrom, double yfrom, double xto,
   double yto, string color) 
{  
   XSetForeground(display, xgc, parse_color(color).pixel);
//   XSetForeground(display, xgc, BlackPixel(display, screen_num));
...
void GraphicWindow::line(double xfrom, double yfrom, double xto,
   double yto) 
{  
   GraphicWindow::line(xfrom, yfrom, xto, yto, "#000000");
}

void GraphicWindow::text(const char t[], double x, double y, string color)
{  
...
{
   GraphicWindow::text(t, x, y, "#000000");
}
...
{  
   ...
   text(_display_string.c_str(), m.get_start().get_x(), m.get_start().get_y(), m.get_color());
...
{  
   line(l.get_start().get_x(), l.get_start().get_y(), 
       l.get_end().get_x(), l.get_end().get_y(), l.get_color());
   return *this;
}
...
string GraphicWindow::get_string(Point p)
{
   XEvent report;
   string instring;
   char instr[50];
